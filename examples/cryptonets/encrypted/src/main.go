...
func pad_image(input [][]uint64) [][]uint64 {
	...
	for i := 0; i < IMAGE_WIDTH; i++ {
		...
		row[IMAGE_WIDTH] = uint64(0)
		output[i] = row
	}
	output[IMAGE_WIDTH] = make([]uint64, padded_width)
	return output
}
...
func (cn CryptoNets) encrypt_images(encoded_images []*rlwe.Plaintext) []*rlwe.Ciphertext {
	...
	for i := range encrypted_images {
		encrypted_images[i] = bfv.NewCiphertext(cn.params, 1, cn.params.MaxLevel())
	}

	for i := 0; i < 841; i++ {
		cn.encryptor.Encrypt(encoded_images[i], encrypted_images[i])
	}
			...
			for stride_x := 0; stride_x < 13; stride_x++ {
				...
				for i := 0; i < 5; i++ {
					for j := 0; j < 5; j++ {
						...
						// column major order
						// fmt.Print(((i+offset_y)*29)+(j+offset_x), " ")
...
func main() {
	...
	for i := 0; i < 8192; i++ {
		for j := 0; j < 845; j++ {
			data := float64(int16(decoded_images[j][i])) / 32.
			fmt.Print(data, " ")
		}
		fmt.Println()
	}
